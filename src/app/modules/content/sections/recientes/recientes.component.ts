...
import { Album, Track } from 'src/app/core/controllers/services/musica/dto/items.dto';
...
export class RecientesComponent {
  ...
  buscarCanciones() {
    this.musicaService.buscarCanciones(this.valorDeBusqueda)
      .subscribe((respuesta) => {
  ...
  seleccionarCancion(cancion: Track) {
    this.musicaService.obtenerAlbum(cancion.album.id)
      .subscribe((album) => {
